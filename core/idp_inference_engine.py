...
from transformers import LayoutLMv3ForTokenClassification, LayoutLMv3Processor # Changed from LayoutLMv2
...
class IDPInferenceEngine:
    ...
    def __init__(
        ...
        self.use_fallback = use_fallback # If true, skip model loading and use regex/keyword
        ...
        self.layoutlm_processor: Optional[LayoutLMv3Processor] = None # Type hint update
        self.layoutlm_model: Optional[LayoutLMv3ForTokenClassification] = None # Type hint update
        ...
        if not self.use_fallback:
            ...
            logger.warning("IDPInferenceEngine initialized in FALLBACK mode. ML models will not be loaded.")

        # Define field mapping for extraction (should match your LayoutLMv3 trainer labels)
        self.id_to_label_map = {
            1: "INVOICE_NUM", 2: "DATE", 3: "DUE_DATE", 4: "VENDOR_NAME", 
            5: "VENDOR_ADDRESS", 6: "CUSTOMER_NAME", 7: "CUSTOMER_ADDRESS", 
            8: "ITEM_DESCRIPTION", 9: "QUANTITY", 10: "UNIT_PRICE", 
            11: "LINE_TOTAL", 12: "SUBTOTAL", 13: "TAX_AMOUNT", 
            14: "DISCOUNT_AMOUNT", 15: "TOTAL_AMOUNT", 16: "CURRENCY", 
            17: "HEADER"
        }
    ...
    def _load_models(self):
        ...
        try:
            ...
            if self.layoutlm_model_path.exists():
                self.layoutlm_processor = LayoutLMv3Processor.from_pretrained(str(self.layoutlm_model_path)) # Changed
                self.layoutlm_model = LayoutLMv3ForTokenClassification.from_pretrained(str(self.layoutlm_model_path)) # Changed
                ...
                if hasattr(self.layoutlm_model.config, 'id2label') and self.layoutlm_model.config.id2label:
                    ...
                    logger.warning("LayoutLMv3 model config has no id2label. Using default internal map.")

            else:
                raise FileNotFoundError(f"LayoutLMv3 model not found at {self.layoutlm_model_path}")
        except Exception as e:
            logger.error(f"Failed to load LayoutLMv3 model: {e}. Falling back to regex extraction.")
        ...
        try:
            ...
            if self.psc_model_path.exists():
                from psc_classifier_trainer import PSCClassifierTrainer # Import locally to avoid circular dep
                _, trainer_instance = PSCClassifierTrainer.load_trained_model(str(self.psc_model_path))
                self.psc_learner = trainer_instance.create_learner(
                    trainer_instance.create_dataloaders(pd.DataFrame([{"text": "dummy", "psc": "Z999"}])) # Dummy dls
                ) # Recreate learner structure to load weights
                self.psc_learner.load_state_dict(torch.load(self.psc_model_path / "learner.pkl").state_dict()) # Load state dict
                ...
                raise FileNotFoundError(f"PSC classifier model not found at {self.psc_model_path}")
        except Exception as e:
            logger.error(f"Failed to load PSC classifier model: {e}. Falling back to keyword matching.")
    ...
    def _parse_layoutlm_predictions(self, words: List[str], boxes: List[List[int]], predictions: List[int]) -> Dict:
        """
        ...
        """
        document_info = {
            "invoice_number": "N/A", "date": "N/A", "due_date": "N/A",
            "vendor_name": "N/A", "vendor_address": "N/A", "customer_name": "N/A",
            "customer_address": "N/A", "subtotal": "N/A", "tax_amount": "N/A",
            "discount_amount": "N/A", "total_amount": "N/A", "currency": "N/A",
            "header": "N/A"
        }
        line_items = []
        ...
        for i, pred_id in enumerate(predictions):
            word = words[i] if i < len(words) else "" # Safeguard against index out of bounds
        ...
        max_items = max(len(v) for v in temp_line_item_components.values()) if temp_line_item_components else 0
        for i in range(max_items):
            item_desc = temp_line_item_components["ITEM_DESCRIPTION"][i] if i < len(temp_line_item_components["ITEM_DESCRIPTION"]) else "N/A"
            qty = temp_line_item_components["QUANTITY"][i] if i < len(temp_line_item_components["QUANTITY"]) else "N/A"
            unit_price = temp_line_item_components["UNIT_PRICE"][i] if i < len(temp_line_item_components["UNIT_PRICE"]) else "N/A"
            line_total = temp_line_item_components["LINE_TOTAL"][i] if i < len(temp_line_item_components["LINE_TOTAL"]) else "N/A"
            line_items.append({
    ...
    def extract_information_layoutlm(self, image_path: str) -> Dict:
        ...
        if not self.layoutlm_model or not self.layoutlm_processor:
            logger.warning("LayoutLMv3 model not loaded. Falling back to regex extraction.")
        ...
        try:
            ...
            if torch.cuda.is_available():
                encoding = {k: v.to(self.layoutlm_model.device) for k, v in encoding.items()}
            ...
            processed_words = words[:seq_len] if len(words) >= seq_len else words # Take up to seq_len
            processed_boxes = boxes[:seq_len] if len(boxes) >= seq_len else boxes
            processed_predictions = predictions[:seq_len] if len(predictions) >= seq_len else processed_predictions # Fixed bug: use predictions[:seq_len]
            ...
            if not processed_words or not processed_boxes or not processed_predictions:
                 logger.warning(f"Empty or malformed processed data for {image_path} after LayoutLMv3.")
                 return {"document_info": {}, "line_items": []}

            return self._parse_layoutlm_predictions(processed_words, processed_boxes, processed_predictions)
    ...
    def classify_psc(self, item_description: str, psc_data: Dict) -> Dict:
        ...
        if self.psc_learner and self.psc_id_to_label:
            try:
                ...
                pred_class_str, pred_idx_tensor, probs_tensor = self.psc_learner.predict(str(item_description))
                
                predicted_psc_code = str(pred_class_str) # This is the PSC string code
                confidence = float(torch.max(probs_tensor)) # Max probability for the predicted class
                ...
                if self.psc_id_to_label and probs_tensor.shape[-1] == len(self.psc_id_to_label):
                    for i, prob in enumerate(probs_tensor.tolist()):
                        probabilities[self.psc_id_to_label.get(i, f"class_{i}")] = float(prob)
                ...
                logger.error(f"PSC model prediction failed for '{item_description}': {e}. Falling back to keyword matching.")
                # Fallback to keyword matching if ML model prediction fails
                return self._keyword_psc_lookup(item_description, psc_data)
        else:
            logger.warning("PSC classifier model not loaded. Using keyword matching for PSC classification.")
            return self._keyword_psc_lookup(item_description, psc_data)
    ...
    def _fallback_extraction(self, image_path: str) -> Dict:
        ...
        invoice_num_match = re.search(r'(invoice|bill|receipt)\s*#?\s*([\w-]+)', full_text, re.IGNORECASE)
        ...
        if total_match:
            ...
            if "$" in total_match.group(2): doc_info["currency"] = "$"
            elif "€" in total_match.group(2): doc_info["currency"] = "€"
            elif "£" in total_match.group(2): doc_info["currency"] = "£"
            elif "¥" in total_match.group(2): doc_info["currency"] = "¥"
        ...
        if len(words) > 5: # If there's enough text
            line_items.append({"item_description": " ".join(words[5:min(len(words), 15)]), "quantity": "1", "unit_price": "N/A", "line_total": "N/A"})
    ...
    def batch_process(self, image_paths: List[str], output_dir: str, psc_data: Dict) -> List[Dict]:
        ...
        logger.info(f"Batch processing completed. Results saved to {batch_output_file}.")
        return results
...
if __name__ == "__main__":
    ...
    if Path(sample_image).exists():
        ...
        sample_text_for_psc = "Purchase of ergonomic office chairs and desks for new employees"
    ...
    if Path(batch_input_folder).exists():
        image_files = [str(f) for f in Path(batch_input_folder).glob("*.jpg")] # Or *.png, etc.
        if image_files:
            ...
            logger.warning(f"No image files found in {batch_input_folder} for batch processing.")
    else:
        logger.warning(f"Batch input folder {batch_input_folder} not found. Skipping batch processing demo.")
