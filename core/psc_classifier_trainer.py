...
class PSCClassifierTrainer:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                "DataPreparationUtils not found. Please ensure it's available."
            )
    ...
    def _initialize_tokenizer(self):
        ...
        try:
            self.tokenizer = AutoTokenizer.from_pretrained(
                self.model_name, use_fast=True, add_prefix_space=False
            )
    ...
    def prepare_psc_training_data(
        ...
        if not psc_data or "psc_mapping" not in psc_data:
            raise ValueError(
                "Invalid PSC data provided: 'psc_mapping' key is missing or empty."
            )
        ...
        for psc_code, psc_info in psc_mapping.items():
            ...
            # Handle lists
            example_phrases_list = [
                str(p) for p in psc_info.get("examplePhrases", []) if p is not None
            ]
            synonyms_list = [
                str(s) for s in psc_info.get("synonyms", []) if s is not None
            ]
            ...
            if (
                spend_category_str
                and spend_category_str != "None"
                and spend_category_str != short_name_str
            ):
                base_description_parts.append(spend_category_str)
            if (
                portfolio_group_str
                and portfolio_group_str != "None"
                and portfolio_group_str not in base_description_parts
            ):
                base_description_parts.append(portfolio_group_str)
            if (
                long_name_str
                and long_name_str != "None"
                and long_name_str not in base_description_parts
            ):
            ...
            unique_base_parts = list(
                dict.fromkeys(
                    [
                        p.strip()
                        for p in base_description_parts
            ...
            if base_text:
                training_samples.append(
                    {"text": base_text, "psc": psc_code_str, "source": "combined_base"}
                )
        ...
        if additional_descriptions:
            for desc_item in additional_descriptions:
                if (
                    ...
                    and "text" in desc_item
                    and "psc" in desc_item
                ):
        ...
        logger.info(
            f"Generated {len(df)} training samples for {df['psc'].nunique()} PSC classes."
        )
        return df
    ...
    def _generate_augmented_samples(
        ...
        context_templates = [
            "Government {item} requirements",
            "Federal {item} contract",
            "Department needs {item}",
            "Agency procurement of {item}",
            "Office {item} purchase",
            "Facility {item} services",
            "Administrative {item}",
            "Operational {item}",
        ]
    ...
    def _clean_training_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Cleaned data: {len(df)} samples for {df['psc'].nunique()} PSC classes."
        )
    ...
    def prepare_data(
        ...
        logger.info(
            f"Prepared {len(df)} training samples for {len(unique_pscs)} PSC classes."
        )
    ...
    def train_model(
        self, df: pd.DataFrame, test_size: float = 0.2, epochs: int = 3
    ) -> Trainer:
        ...
        # Get training arguments with version compatibility
        training_args_kwargs = self._get_training_args_kwargs(
    ...
    def evaluate_model(self, trainer: Trainer, df: pd.DataFrame = None):
        ...
        if df is not None:
            ...
            for text in test_cases:
                ...
                logger.info(
                    f"Prediction: {result['predicted_psc']} (Confidence: {result['confidence']:.3f})"
                )
                logger.info("-" * 50)
            ...
            with torch.no_grad():
                ...
                if top_k == 1:
                    ...
                    for i in range(top_k):
                        ...
                        results.append({"predicted_psc": psc, "confidence": conf})

                    return {"predictions": results, "text": text}
    ...
    def load_model(cls, path: str):
        ...
        # Initialize trainer
        trainer = cls(
            model_name=mappings["model_name"], max_length=mappings["max_length"]
        )
        ...
        logger.info(
            f"Model supports {mappings.get('num_labels', 'unknown')} PSC classes"
        )
        return trainer
...
def train_psc_classifier():
    ...
    try:
        ...
        logger.info(
            f"Loaded {len(psc_data.get('psc_mapping', {}))} PSC codes across {len(set([v.get('spendCategoryParent', '') for v in psc_data.get('psc_mapping', {}).values()]))} categories"
        )
