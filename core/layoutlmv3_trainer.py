...
class KaggleInvoiceDataLoader:
    ...
    def __init__(self, annotations_dir: str, images_dir: str, output_dir: str):
        ...
        self.base_labels = [
            ...
            "ITEM_DESCRIPTION",
            "QUANTITY",
            "UNIT_PRICE",  # UNIT_PRICE (from B-PRICE)
            "LINE_TOTAL",
            "SUBTOTAL",
            "TAX_AMOUNT",  # TAX_AMOUNT (from B-TAX)
            "DISCOUNT_AMOUNT",
            "TOTAL_AMOUNT",
            "CURRENCY",
            "HEADER",
    ...
    def load_annotations(self) -> List[Dict]:
        ...
        for json_path in json_files:
            try:
                with open(json_path, "r", encoding="utf-8") as f:
                    ...
                    if "image_path" in data and data["image_path"]:
                        ...
                        if potential_path.exists():
                            ...
                            logger.error(
                                f"Image specified in JSON '{data['image_path']}' ({potential_path}) does not exist for {json_path.name}. Attempting inference."
                            )

                    if (
                        image_full_path is None
                    ):  # If not found via JSON path, or if JSON path was missing/invalid
                        ...
                        for ext in [
                            ...
                            potential_image_path = (
                                self.images_dir / f"{image_filename_stem}{ext}"
                            )
                            if potential_image_path.exists():
                                image_full_path = str(potential_image_path)
                                logger.info(
                                    f"Inferred image_path for {json_path.name}: {image_full_path}"
                                )
                                break

                    if image_full_path is None:
                        logger.error(
                            f"Could not find any corresponding image for {json_path.name} in {self.images_dir}. Skipping annotation."
                        )
                    ...
                    try:
                        ...
                        logger.error(
                            f"Error loading image {image_full_path} to get dimensions: {e}. Skipping."
                        )
                        continue
                    ...
                    for i, bbox in enumerate(incoming_boxes):
                        try:
                            ...
                            fixed_bbox = validate_and_fix_bbox(
                                bbox, img_width, img_height
                            )
                            # Then normalize to 0-1000 range
                            normalized_bbox = normalize_bbox(
                                fixed_bbox, img_width, img_height
                            )
                            normalized_boxes.append(normalized_bbox)
                        except Exception as e:
                            logger.warning(
                                f"Error processing bbox {bbox} for word '{incoming_words[i] if i < len(incoming_words) else 'N/A'}' in {json_path.name}: {e}. Using default bbox."
                            )
                    ...
                    for tag in incoming_ner_tags:
                        if tag == "O":
                            ...
                            if original_label in self.ner_tag_translation_map:
                                translated_label = self.ner_tag_translation_map[
                                    original_label
                                ]
                                translated_ner_tags.append(
                                    f"{prefix}-{translated_label}"
                                )
                            elif (
                                original_label in self.base_labels
                            ):  # Already a direct match
                                translated_ner_tags.append(tag)
                            else:
                                logger.warning(
                                    f"Unknown NER tag '{tag}' in {json_path.name}. Mapping to 'O'."
                                )
                                translated_ner_tags.append(
                                    "O"
                            ...
                            logger.warning(
                                f"Malformed NER tag '{tag}' in {json_path.name}. Mapping to 'O'."
                            )
                            translated_ner_tags.append(
                                "O"
                            )  # Default to 'O' for malformed tags
                    ...
                    if (
                        not processed_item["tokens"]
                        or not processed_item["bboxes"]
                        or not processed_item["ner_tags"]
                    ):
                        logger.warning(
                            f"Skipping {json_path.name}: Missing 'tokens', 'bboxes', or 'ner_tags' data after processing."
                        )
                        continue
                    if not (
                        len(processed_item["tokens"])
                        == len(processed_item["bboxes"])
                        == len(processed_item["ner_tags"])
                    ):
                        logger.warning(
                            f"Skipping {json_path.name}: Mismatched lengths in 'tokens' ({len(processed_item['tokens'])}), 'bboxes' ({len(processed_item['bboxes'])}), or 'ner_tags' ({len(processed_item['ner_tags'])}) after processing."
                        )
                        continue
                ...
                logger.error(
                    f"Error processing annotation file {json_path}: {e}. Skipping."
                )
        return all_data

    def get_train_val_split(
        self, data: List[Dict], val_split: float = 0.1
        ...
        logger.info(
            f"Data split: {len(train_data)} training samples, {len(val_data)} validation samples."
        )
    ...
    def print_data_statistics(self, data: List[Dict]):
        ...
        if bbox_stats["min_vals"] and bbox_stats["max_vals"]:
            logger.info(
                f"Bbox coordinate range: {min(bbox_stats['min_vals'])} to {max(bbox_stats['max_vals'])}"
            )
...
class LayoutLMv3Learner:
    ...
    def compute_metrics(
        ...
        true_predictions = [
            label_id_to_name_map[p]
            for prediction, label in zip(predictions, labels)
            for p, l in zip(prediction, label)
            if l != -100
        ]
        true_labels = [
            label_id_to_name_map[l]
            for prediction, label in zip(predictions, labels)
            for p, l in zip(prediction, label)
            if l != -100
        ]
        ...
        per_label_f1 = {
            ...
            for label in report
            if label != "O" and isinstance(report[label], dict)
        }
    ...
    def train(
        ...
        logger.info(
            f"Fine-tuned model and processor saved to {self.output_dir / 'fine_tuned_layoutlmv3'}"
        )
...
def main():
    ...
    try:
        kaggle_data_loader = KaggleInvoiceDataLoader(
            KAGGLE_ANNOTATIONS_DIR, KAGGLE_IMAGES_DIR, OUTPUT_DIR
        )
        synthetic_data_loader = KaggleInvoiceDataLoader(
            SYNTHETIC_ANNOTATIONS_DIR, SYNTHETIC_IMAGES_DIR, OUTPUT_DIR
        )
        ...
        if not all_data:
            logger.error(
                "No valid data found. Please check your annotation files and images paths."
            )
            return

        kaggle_data_loader.print_data_statistics(all_data)

        train_data, val_data = kaggle_data_loader.get_train_val_split(
            all_data, VAL_SPLIT
        )

        learner = LayoutLMv3Learner(
            num_labels=kaggle_data_loader.num_labels, output_dir=OUTPUT_DIR
        )

        train_dataset = LayoutLMDataset(
            train_data, learner.processor, MAX_LENGTH, kaggle_data_loader.id_to_label
        )
        val_dataset = LayoutLMDataset(
            val_data, learner.processor, MAX_LENGTH, kaggle_data_loader.id_to_label
        )
