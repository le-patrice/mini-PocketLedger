...
class TextNormalizer:
    ...
    def normalize_date(text: str) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Could not parse date with dateutil: '{text}'. Falling back to simple clean."
            )
    ...
    def normalize_text(text: str, field_type: str = "text") -> str:
        ...
        if field_type == "number":
            return TextNormalizer.normalize_number(text)
        if field_type == "date":
            return TextNormalizer.normalize_date(text)
...
class AnnotationGenerator:
                ...
                if confidence > 0.3 and text.strip():
                    ...
                    if clean_text:
                        word_level_data.append(
                            (clean_text, [x_min, y_min, x_max, y_max], confidence)
                        )

            logger.debug(
                f"OCR extracted {len(word_level_data)} words from {image_path.name}"
            )
                    ...
                    if (
                        ...
                        if entity_label:
                            ...
                            if field == "description":
                                ...
                                entities.append(
                                    (str(item[field]).strip(), entity_label)
                                )
    ...
    def find_best_match(
        ...
        gt_text: str,
        label: str,
        ocr_data: List[Tuple[str, List[int], float]],
        used_indices: set,
    ) -> List[int]:
        """
        ...
        field_type = (
            ...
            else "date" if label in DATE_FIELDS else "text"
        )
        gt_norm = self.normalizer.normalize_text(gt_text, field_type)
        gt_words_norm_list = [
            self.normalizer.normalize_text(w, field_type) for w in gt_text.split() if w
        ]

        # Caching normalized OCR words for efficiency in loops
        normalized_ocr_words = [
            self.normalizer.normalize_text(ocr_word, field_type)
            for ocr_word, _, _ in ocr_data
        ]
        ...
        if (
            len(gt_words_norm_list) == 1 and len(gt_text.split()) == 1
        ):  # Ensure it's truly a single word GT
            for i, ocr_norm_word in enumerate(normalized_ocr_words):
                if i in used_indices:
                    continue
                if gt_norm == ocr_norm_word:
                    logger.debug(
                        f"STRATEGY 1 (Exact Single Word) match for '{gt_text}' ({label}): '{ocr_data[i][0]}' at index {i}"
                    )
                    return [i]
        elif len(gt_words_norm_list) > 1:  # Multi-word GT, try strict sequence
            for start in range(len(ocr_data) - len(gt_words_norm_list) + 1):
                current_ocr_indices = list(
                    range(start, start + len(gt_words_norm_list))
                )
                if any(idx in used_indices for idx in current_ocr_indices):
                    continue

                # Check normalized words directly
                ocr_sub_sequence_norm = normalized_ocr_words[
                    start : start + len(gt_words_norm_list)
                ]
                if gt_words_norm_list == ocr_sub_sequence_norm:
                    logger.debug(
                        f"STRATEGY 1 (Exact Multi-Word Sequence) match for '{gt_text}' ({label}): '{' '.join([ocr_data[i][0] for i in current_ocr_indices])}' at {current_ocr_indices}"
                    )
        ...
        for window_size in range(1, max_window_size + 1):
            for start in range(len(ocr_data) - window_size + 1):
                ...
                ocr_segment_norm = " ".join(
                    normalized_ocr_words[start : start + window_size]
                )
                ...
                if score > best_score_overall:  # Always keep the highest score
                    ...
                    logger.debug(
                        f"STRATETERY 2 (Token Set) candidate for '{gt_text}' ({label}): '{' '.join([ocr_data[i][0] for i in current_ocr_indices])}' with score {score} at {current_ocr_indices}"
                    )

        # Final decision based on the best overall score from token set ratio
        # Set minimum threshold for a valid match
        final_threshold = (
            90 if is_critical else 75
        )  # Adjust as needed based on empirical results

        if best_score_overall >= final_threshold and best_indices_overall:
            logger.debug(
                f"Best Token Set Match for '{gt_text}' ({label}): indices {best_indices_overall} (Score: {best_score_overall})"
            )
            return best_indices_overall

        logger.debug(
            f"No good match found for ground truth: '{gt_text[:50]}...' (Label: {label})"
        )
        return []

    def align_ocr_with_ground_truth(
        self, ocr_data: List[Tuple[str, List[int], float]], ground_truth: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        for gt_text, entity_label in gt_entities:
            match_indices = self.find_best_match(
                gt_text, entity_label, ocr_data, used_indices
            )

            # Filter out indices that have already been tagged by a higher priority entity
            available_indices = [
                idx for idx in match_indices if idx not in used_indices
            ]
            ...
            if available_indices:
                ...
                for i, idx in enumerate(available_indices):
                    ...
                    if ner_tags[idx] == "O":
                        ...
                        logger.debug(
                            f"OCR word {idx} ('{ocr_data[idx][0]}') already tagged as '{ner_tags[idx]}'. "
                            f"Skipping re-tagging for '{entity_label}' ('{gt_text}')."
                        )

                logger.debug(
                    f"Tagged '{gt_text[:30]}...' ({entity_label}) at indices {available_indices}"
                )
            else:
                logger.debug(
                    f"No match for GT: '{gt_text[:30]}...' (Label: {entity_label})."
                )

        tagged_count = sum(1 for tag in ner_tags if tag != "O")
        logger.info(
            f"Alignment: Tagged {tagged_count}/{num_ocr_words} OCR words ({tagged_count/num_ocr_words*100:.1f}% coverage)"
        )
    ...
    def extract_invoice_metadata(self, ground_truth: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_str_from_gt:
            try:
                ...
                normalized_total_str = self.normalizer.normalize_number(
                    total_str_from_gt
                )
                total_amount = (
                    float(normalized_total_str) if normalized_total_str else 0.0
                )
                logger.debug(
                    f"Extracted total from subtotal.total in GT: {total_amount}"
                )
            except ValueError:
                logger.warning(
                    f"Could not parse total_amount from 'subtotal.total' in GT: '{total_str_from_gt}'."
                )
        ...
        if total_amount == 0.0 or np.isnan(total_amount):
            ...
            for item in ground_truth.get("items", []):
                if (
                    ...
                    and "total_price" in item
                    and item["total_price"] is not None
                ):
                    try:
                        # IMPORTANT FIX: Same handling for line item total_price
                        normalized_price_str = self.normalizer.normalize_number(
                            str(item["total_price"])
                        )
                        price = (
                            float(normalized_price_str) if normalized_price_str else 0.0
                        )
                        calculated_total += price
                    except ValueError:
                        logger.warning(
                            f"Could not parse line item total_price for calculation: '{item.get('total_price')}'"
                        )

            if calculated_total > 0:
                total_amount = calculated_total
                logger.info(
                    f"Used calculated total from line items as fallback: ${total_amount:.2f}"
                )

        # IMPORTANT FIX: Apply same robust conversion for tax_amount and discount_amount
        tax_amount = 0.0
        normalized_tax_str = self.normalizer.normalize_number(
            subtotal_data.get("tax", "0.0")
        )
        try:
        ...
        discount_amount = 0.0
        normalized_discount_str = self.normalizer.normalize_number(
            subtotal_data.get("discount", "0.0")
        )
        try:
            discount_amount = (
                float(normalized_discount_str) if normalized_discount_str else 0.0
            )
        except ValueError:
            logger.warning(
                f"Could not parse discount_amount: '{normalized_discount_str}'"
            )

        return {
            "invoice_id": invoice_data.get("invoice_number", ""),
            "date": self.normalizer.normalize_date(
            ...
            "due_date": self.normalizer.normalize_date(
    ...
    def generate_annotation(
        self, image_path: Path, json_data_str: str, output_dir: Path
        ...
        try:
            ...
            if not ocr_results:
                logger.warning(
                    f"No valid OCR results for {image_path.name}. Annotation skipped."
                )
                return None
            ...
            if not (len(words) == len(boxes) == len(ner_tags) == len(confidences)):
                logger.error(
                    f"Length mismatch after alignment for {image_path.name}. Words={len(words)}, Boxes={len(boxes)}, Tags={len(ner_tags)}, Confidences={len(confidences)}",
                    exc_info=True,
                )
            ...
            annotation = {
                ...
                "alignment_stats": {
                    ...
                    "tag_coverage": (
                        (sum(1 for tag in ner_tags if tag != "O") / len(words))
                        if words
                        else 0
                    ),
                },
            ...
            logger.info(
                f"Generated {output_file.name} (Coverage: {coverage:.1%}, Total: ${total:.2f})"
            )
            ...
            logger.error(
                f"Failed to generate annotation for {image_path.name} due to unexpected error: {e}",
                exc_info=True,
            )
            self.stats["failed_alignment"] += 1
            return None
    ...
    def print_summary(self):
        ...
        logger.info(
            f"Failed (Alignment/General errors): {self.stats['failed_alignment']}"
        )
        logger.info(
            f"Skipped (Missing image files): {self.stats['skipped_missing_image']}"
        )
...
def main():
    ...
    try:
        ...
        logger.critical(
            f"FATAL: Annotation generation process failed unexpectedly: {e}",
            exc_info=True,
        )
        return 1
